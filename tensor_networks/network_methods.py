...
def _add_vecs(xx, yy, lattice_shape):
...
def _crystal_sites(lattice_shape, lattice_vectors):
    ...
    for lattice_vector in lattice_vectors:
        for base_loc in locations:
            new_loc = _add_vecs(base_loc, lattice_vector, lattice_shape)
...
def crystal_network(lattice_shape, lattice_vectors, tensor_bundle):
    ...
    for base_loc in tensor_locations: # for every tensor location
        ...
        for axis, lattice_vector in enumerate(lattice_vectors):
            ...
            trgt_loc = _add_vecs(base_loc, lattice_vector, lattice_shape)
...
def crystal_bubbler(lattice_shape, lattice_vectors):
    ...
    def _adjacent_nodes(node):
        return ( [ _add_vecs(node, step, lattice_shape)
                   for step in lattice_vectors ] +
                 [ _add_vecs(node, -np.array(step), lattice_shape)
                   for step in lattice_vectors ] )
...
def cubic_network(lattice_shape, tensor_bundle):
    ...
    return crystal_network(lattice_shape, lattice_vectors, tensor_bundle)

def cubic_bubbler(lattice_shape):
    lattice_vectors = _cubic_lattice_vectors(lattice_shape)
    return crystal_bubbler(lattice_shape, lattice_vectors)
...
def checkerboard_bubbler(lattice_shape):
    lattice_vectors = _checkerboard_lattice_vectors(lattice_shape)
    return crystal_bubbler(lattice_shape, lattice_vectors)
